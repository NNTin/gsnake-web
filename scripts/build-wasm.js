...
const isRootRepo =
  !forceGitDeps && existsSync(rootGitPath) && existsSync(gsnakeCoreCargoPath);
...
if (isRootRepo) {
  console.log(
    "[build-wasm] ✓ Root repository detected - building WASM from local gsnake-core",
  );

  if (!existsSync(buildWasmScript)) {
    console.error(
      "[build-wasm] ❌ ERROR: build_wasm.py not found at",
      buildWasmScript,
    );
    process.exit(1);
  }
  ...
  if (result.error) {
    console.error(
      "[build-wasm] ❌ ERROR: Failed to execute build_wasm.py:",
      result.error,
    );
    process.exit(1);
  }
  ...
  if (result.status !== 0) {
    console.error(
      "[build-wasm] ❌ ERROR: build_wasm.py exited with status",
      result.status,
    );
  ...
  console.log(
    "[build-wasm] ⚠️  Standalone mode detected - skipping WASM build",
  );
